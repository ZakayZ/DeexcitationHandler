...
G4StatMFMacroTriNucleon::G4StatMFMacroTriNucleon() : G4VStatMFMacroCluster(3) {}

G4StatMFMacroTriNucleon::~G4StatMFMacroTriNucleon() {}

G4double G4StatMFMacroTriNucleon::CalcMeanMultiplicity(const G4double FreeVol, const G4double mu, const G4double nu,
                                                       const G4double T) {
  ...
  G4double exponent =
      (BindingE + theA * (mu + nu * theZARatio) -
...
G4double G4StatMFMacroTriNucleon::CalcEntropy(const G4double T, const G4double FreeVol) {
  ...
  if (_MeanMultiplicity > 0.0) {
    G4double ThermalWaveLenght = 16.15 * fermi / std::sqrt(T);
    G4double lambda3 = ThermalWaveLenght * ThermalWaveLenght * ThermalWaveLenght;
    Entropy = _MeanMultiplicity *
              (2.5 + G4Log((4 * theA) * std::sqrt((G4double)theA) * FreeVol / (lambda3 * _MeanMultiplicity)));
  }
  return Entropy;
}
