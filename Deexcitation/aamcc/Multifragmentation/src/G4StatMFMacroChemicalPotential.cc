...
G4StatMFMacroChemicalPotential& G4StatMFMacroChemicalPotential::operator=(const G4StatMFMacroChemicalPotential&) {
...
//	Calculate Chemical potential \nu
{
  ...
  _ChemPotentialNu = (theZ / theA) * (8.0 * G4StatMFParameters::GetGamma0() + 2.0 * CP * g4calc->Z23(theA)) -
  ...
  if (!theSolver->Brent(*this)) {
    G4cout << "G4StatMFMacroChemicalPotential:" << " ChemPa=" << ChemPa << " ChemPb=" << ChemPb << G4endl;
    G4cout << "G4StatMFMacroChemicalPotential:" << " fChemPa=" << fChemPa << " fChemPb=" << fChemPb << G4endl;
    throw G4HadronicException(__FILE__, __LINE__,
...
// For that is necesary to calculate mean multiplicities
{
  G4StatMFMacroMultiplicity* theMultip =
      new G4StatMFMacroMultiplicity(theA, _Kappa, _MeanTemperature, nu, _theClusters);
