...
G4StatMFMacroMultiplicity& G4StatMFMacroMultiplicity::operator=(const G4StatMFMacroMultiplicity&) {
...
// For that is necesary to calculate mean multiplicities
{
  ...
  _ChemPotentialMu = -G4StatMFParameters::GetE0() - _MeanTemperature * _MeanTemperature / ILD5 -
                     _ChemPotentialNu * ZA5 + G4StatMFParameters::GetGamma0() * (1.0 - 2.0 * ZA5) * (1.0 - 2.0 * ZA5) +
                     (2.0 / 3.0) * G4StatMFParameters::Beta(_MeanTemperature) / g4calc->Z13(5) +
                     (5.0 / 3.0) * CP * ZA5 * ZA5 * g4calc->Z23(5) - 1.5 * _MeanTemperature / 5.0;
  ...
  {
    G4cout << "G4StatMFMacroMultiplicity:" << " ChemPa=" << ChemPa << " ChemPb=" << ChemPb << G4endl;
    G4cout << "G4StatMFMacroMultiplicity:" << " fChemPa=" << fChemPa << " fChemPb=" << fChemPb << G4endl;
...
G4double G4StatMFMacroMultiplicity::CalcMeanA(const G4double mu) {
  ...
  G4int n = 1;
  for (std::vector<G4VStatMFMacroCluster*>::iterator i = _theClusters->begin(); i != _theClusters->end(); ++i) {
