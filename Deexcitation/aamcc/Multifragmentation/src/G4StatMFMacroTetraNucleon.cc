...
G4StatMFMacroTetraNucleon::G4StatMFMacroTetraNucleon() : G4VStatMFMacroCluster(4) {}

G4StatMFMacroTetraNucleon::~G4StatMFMacroTetraNucleon() {}

G4double G4StatMFMacroTetraNucleon::CalcMeanMultiplicity(const G4double FreeVol, const G4double mu, const G4double nu,
                                                         const G4double T) {
  ...
  G4double exponent =
      (BindingE + theA * (mu + nu * theZARatio + T * T / _InvLevelDensity) -
       G4StatMFParameters::GetCoulomb() * theZARatio * theZARatio * theA * G4Pow::GetInstance()->Z23(theA)) /
      T;
  if (exponent > 300.0) exponent = 300.0;

  _MeanMultiplicity = (degeneracy * FreeVol * theA * std::sqrt((G4double)theA) / lambda3) * G4Exp(exponent);
...
G4double G4StatMFMacroTetraNucleon::CalcEntropy(const G4double T, const G4double FreeVol) {
  ...
  if (_MeanMultiplicity > 0.0) {
    ...
    G4double lambda3 = ThermalWaveLenght * ThermalWaveLenght * ThermalWaveLenght;
    Entropy =
        _MeanMultiplicity * (2.5 + G4Log(8 * FreeVol / (lambda3 * _MeanMultiplicity))) + 8.0 * T / _InvLevelDensity;
  }
  return Entropy;
}
