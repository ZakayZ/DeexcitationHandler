...
class SumCoulombEnergy {
 public:
  SumCoulombEnergy(G4int anA, G4int anZ) : initA(anA), initZ(anZ) {}

  G4double operator()(G4double total, const G4StatMFFragment* frag) const {
    return total + frag->GetCoulombEnergy(initA, initZ);
  }
...
G4StatMFChannel::G4StatMFChannel() : _NumOfNeutralFragments(0), _NumOfChargedFragments(0) {}
...
G4double G4StatMFChannel::GetFragmentsCoulombEnergy(G4int initA, G4int initZ) {
  G4double Coulomb = std::accumulate(_theFragments.begin(), _theFragments.end(), 0.0, SumCoulombEnergy(initA, initZ));
  return Coulomb;
}
...
G4FragmentVector* G4StatMFChannel::GetFragments(G4int anA, G4int anZ, G4double T) {
  if (_NumOfChargedFragments > 1) {
    ...
    throw G4HadronicException(
        __FILE__, __LINE__,
        "G4StatMFChannel::GetFragments: only 1 fragment to deal with, multifragmentation didn't happen");
  } else {
...
// After breakup, fragments fly away under Coulomb field.
...
// Fragments positions are sampled inside prolongated ellipsoid.
{
  ...
  do {
    ...
    G4double R = (Rsys - R0 * g4calc->Z13(_theFragments[0]->GetA())) * g4calc->A13(G4UniformRand());
    ...
    for (i = _theFragments.begin() + 1; i != _theFragments.end(); ++i) {
      ...
      do {
        ...
        for (j = _theFragments.begin(); j != i; ++j) {
          ...
          G4double Rmin = R0 * (g4calc->Z13((*i)->GetA()) + g4calc->Z13((*j)->GetA()));
...
// idx is index of first fragment
{
  ...
  if (NF <= 0)
    ...
    p = std::sqrt(2.0 * _theFragments[idx]->GetNuclearMass() * KinE) * G4RandomDirection();
    ...
    do {
      ...
      for (G4int i = idx; i < idx + NF - 2; ++i) {
        ...
        while (RandE > 1.0);
        E *= T;
        p = std::sqrt(2.0 * E * _theFragments[i]->GetNuclearMass()) * G4RandomDirection();
    ...
    // Loop checking, 05-Aug-2015, Vladimir Ivanchenko
    while (AvailableE <=
           p.mag2() / (2.0 * (_theFragments[i1]->GetNuclearMass() + _theFragments[i2]->GetNuclearMass())));
    ...
    if (CTM12 > 1.) {
      ...
      // Loop checking, 05-Aug-2015, Vladimir Ivanchenko
      while (CTM12 >= 0.0 && CosTheta1 < 0.0);  // this should never work
    }
...
// J.F. Bondorf et al. Physics Reports 257 (1995) 133-221
{
  ...
  G4double CoulombEnergy =
      0.6 * CLHEP::elm_coupling * CoulombFactor * anZ * anZ / (G4StatMFParameters::Getr0() * g4calc->Z13(anA)) -
      GetFragmentsCoulombEnergy(anA, anZ);
    ...
    for (i = 0; i < _NumOfChargedFragments; i++) {
      ...
      for (G4int j = 0; j < _NumOfChargedFragments; j++) {
        if (i != j) {
          ...
          force += (1.44 * pow(10, -24) * _theFragments[i]->GetZ() * _theFragments[j]->GetZ() /
                    (distance.mag2() * distance.mag())) *
                   distance;
        }
      }
...
// Rotates a 3-vector P to close momentum triangle Pa + V + P = 0
{
  ...
  G4ThreeVector RotatedMomentum(((V.x() - Alpha1 * U.x()) / Alpha2) * P.x() + N.x() * P.y() + U.x() * P.z(),
