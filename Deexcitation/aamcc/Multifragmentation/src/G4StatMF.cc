...
G4FragmentVector* G4StatMF::BreakItUp(const G4Fragment& theFragment) {
  ...
  G4double MaxAverageMultiplicity = G4StatMFParameters::GetMaxAverageMultiplicity(theFragment.GetA_asInt());
  ...
  G4FragmentVector* theResult =
      theChannel->GetFragments(theFragment.GetA_asInt(), theFragment.GetZ_asInt(), Temperature);
  ...
  do {
    ...
    for (j = theResult->begin(); j != theResult->end(); j++) FragmentsEnergy += (*j)->GetMomentum().e();
...
G4bool G4StatMF::FindTemperatureOfBreakingChannel(const G4Fragment& theFragment, const G4StatMFChannel* aChannel,
...
G4double G4StatMF::CalcEnergy(G4int A, G4int Z, const G4StatMFChannel* aChannel, G4double T) {
  ...
  G4double CoulombEnergy =
      0.6 * CLHEP::elm_coupling * CoulombFactor * Z * Z / (G4StatMFParameters::Getr0() * g4calc->Z13(A));
