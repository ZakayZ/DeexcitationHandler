...
G4StatMFMacroTemperature::G4StatMFMacroTemperature(const G4double anA, const G4double aZ, const G4double ExEnergy,
...
G4double G4StatMFMacroTemperature::CalcTemperature(void) {
  ...
  if (std::fabs(FunctionValureAtRoot) > 5.e-2) {
    if (_MeanTemperature < 1. || _MeanTemperature > 50.) {
      G4cout << "Crenshaw method failed; function = " << FunctionValureAtRoot << " solution? = " << _MeanTemperature
             << " MeV " << G4endl;
      ...
      if (!theSolverBrent->Brent(*this)) {
        ...
        throw G4HadronicException(
            __FILE__, __LINE__, "G4StatMFMacroTemperature::CalcTemperature: I couldn't find the root with any method.");
      }
    ...
    if (std::abs(FunctionValureAtRoot) > 5.e-2) {
      G4cout << "Brent method failed; function = " << FunctionValureAtRoot << " solution? = " << _MeanTemperature
             << " MeV " << G4endl;
      throw G4HadronicException(__FILE__, __LINE__,
...
{
  G4StatMFMacroChemicalPotential* theChemPot = new G4StatMFMacroChemicalPotential(theA, theZ, _Kappa, T, _theClusters);
