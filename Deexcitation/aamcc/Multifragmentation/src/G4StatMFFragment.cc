...
G4StatMFFragment& G4StatMFFragment::operator=(const G4StatMFFragment&) {
...
G4double G4StatMFFragment::GetCoulombEnergy(const G4int initA, const G4int initZ) const {
  G4double res = 0.0;
  G4Pow* g4calc = G4Pow::GetInstance();
  if (theZ >= 1) {
    res = 0.6 * CLHEP::elm_coupling * theZ * theZ /
          (G4StatMFParameters::Getr0() * g4calc->A23(1.0 + G4StatMFParameters::GetKappaCoulomb())) *
          g4calc->A13(G4double(initZ) / (initA * theZ));
  }
  return res;
}
...
G4double G4StatMFFragment::GetEnergy(const G4double T, const G4int initA, const G4int initZ) const {
  if (theA < 1 || theZ < 0 || theZ > theA) {
    G4cout << "G4StatMFFragment::GetEnergy: A = " << theA << ", Z = " << theZ << G4endl;
    throw G4HadronicException(__FILE__, __LINE__, "G4StatMFFragment::GetEnergy: Wrong values for A and Z!");
  }
  ...
  if (G4StatMFParameters::DBetaDT(T) == 0.0)
    SurfaceEnergy = 0.0;
  else
    SurfaceEnergy = 2.5 * G4Pow::GetInstance()->Z23(theA) * T * T * G4StatMFParameters::GetBeta0() /
...
G4double G4StatMFFragment::CalcExcitationEnergy(const G4double T) {
  ...
  if (std::abs(q) > 1.0e-20) {
    SurfaceEnergy =
        2.5 * G4Pow::GetInstance()->Z23(theA) * (G4StatMFParameters::Beta(T) - T * q - G4StatMFParameters::GetBeta0());
  }
