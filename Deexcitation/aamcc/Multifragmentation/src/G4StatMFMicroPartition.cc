...
G4StatMFMicroPartition& G4StatMFMicroPartition::operator=(const G4StatMFMicroPartition&) {
...
void G4StatMFMicroPartition::CoulombFreeEnergy(G4int anA) {
  ...
  if (anA == 0 || anA == 1) {
    ...
    // Z/A ~ 1/2
    _theCoulombFreeEnergy.push_back(CoulombConstFactor * 0.5 * anA * G4Pow::GetInstance()->Z23(anA));
  } else  // anA > 4
  {
    _theCoulombFreeEnergy.push_back(CoulombConstFactor * ZA * ZA * anA * G4Pow::GetInstance()->Z23(anA));
  }
}
...
G4double G4StatMFMicroPartition::GetCoulombEnergy(void) {
  ...
  G4double CoulombEnergy =
      elm_coupling * 0.6 * theZ * theZ * CoulombFactor / (G4StatMFParameters::Getr0() * g4calc->Z13(theA));

  G4double ZA = G4double(theZ) / G4double(theA);
  for (unsigned int i = 0; i < _thePartition.size(); i++)
    CoulombEnergy += _theCoulombFreeEnergy[i] - elm_coupling * 0.6 * ZA * ZA * _thePartition[i] *
                                                    g4calc->Z23(_thePartition[i]) / G4StatMFParameters::Getr0();
...
G4double G4StatMFMicroPartition::GetPartitionEnergy(G4double T) {
  ...
  for (unsigned int i = 0; i < _thePartition.size(); i++) {
    if (_thePartition[i] == 0 || _thePartition[i] == 1) {
      ...
      PartitionEnergy += -2.796  // Binding Energy of deuteron ??????
                         + _theCoulombFreeEnergy[i];
    } else if (_thePartition[i] == 3) {
      PartitionEnergy += -9.224  // Binding Energy of trtion/He3 ??????
                         + _theCoulombFreeEnergy[i];
    } else if (_thePartition[i] == 4) {
      PartitionEnergy += -30.11  // Binding Energy of ALPHA ??????
                         + _theCoulombFreeEnergy[i] + 4. * T * T / InvLevelDensity(4.);
    } else {
      PartitionEnergy +=
          // Volume term
          (-G4StatMFParameters::GetE0() + T * T / InvLevelDensity(_thePartition[i])) * _thePartition[i] +

          // Symmetry term
          G4StatMFParameters::GetGamma0() * (1.0 - 2.0 * theZ / theA) * (1.0 - 2.0 * theZ / theA) * _thePartition[i] +

          // Surface term
          (G4StatMFParameters::Beta(T) - T * G4StatMFParameters::DBetaDT(T)) * g4calc->Z23(_thePartition[i]) +
...
G4double G4StatMFMicroPartition::CalcPartitionTemperature(G4double U, G4double FreeInternalE0) {
  ...
  G4cout << "G4StatMFMicroPartition::CalcPartitionTemperature: I can't calculate the temperature" << G4endl;
  // and set probability to 0 returning T < 0
  return -1.0;
}

G4double G4StatMFMicroPartition::CalcPartitionProbability(G4double U, G4double FreeInternalE0, G4double SCompound) {
  ...
  for (i = 0; i < _thePartition.size(); i++) {
    ...
    if (_thePartition[i] == 4) {
      PartitionEntropy += 2.0 * T * _thePartition[i] / InvLevelDensity(_thePartition[i]);
    }
  ...
  G4double kappa =
      1. + elm_coupling * (g4calc->Z13(_thePartition.size()) - 1.0) / (G4StatMFParameters::Getr0() * g4calc->Z13(theA));
  kappa = kappa * kappa * kappa;
  kappa -= 1.;
  G4double V0 =
      (4. / 3.) * pi * theA * G4StatMFParameters::Getr0() * G4StatMFParameters::Getr0() * G4StatMFParameters::Getr0();
  G4double FreeVolume = kappa * V0;
  G4double TranslationalS =
      std::max(0.0, G4Log(ProbA32 / Fact) + (_thePartition.size() - 1.0) * G4Log(FreeVolume / ThermalWaveLenght3) +
...
G4double G4StatMFMicroPartition::GetDegeneracyFactor(G4int A) {
  ...
  if (A > 4)
    ...
    DegFactor = 4.0;  // nucleon
  else if (A == 2)
    DegFactor = 3.0;  // Deuteron
  else if (A == 3)
    DegFactor = 4.0;  // Triton + He3
  else if (A == 4)
    DegFactor = 1.0;  // alpha
  return DegFactor;
}
