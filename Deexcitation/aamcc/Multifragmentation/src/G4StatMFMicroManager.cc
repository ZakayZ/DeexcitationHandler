...
G4StatMFMicroManager& G4StatMFMicroManager::operator=(const G4StatMFMicroManager&) {
  throw G4HadronicException(__FILE__, __LINE__, "G4StatMFMicroManager::operator= meant to not be accessible");
  return *this;
}

G4bool G4StatMFMicroManager::operator==(const G4StatMFMicroManager&) const { return false; }

G4bool G4StatMFMicroManager::operator!=(const G4StatMFMicroManager&) const { return true; }

// constructor
G4StatMFMicroManager::G4StatMFMicroManager(const G4Fragment& theFragment, G4int multiplicity, G4double FreeIntE,
                                           G4double SCompNuc)
    : _Normalization(0.0) {
...
G4StatMFMicroManager::~G4StatMFMicroManager() {
  if (!_Partition.empty()) {
    std::for_each(_Partition.begin(), _Partition.end(), DeleteFragment());
  }
}

void G4StatMFMicroManager::Initialize(const G4Fragment& theFragment, G4int im, G4double FreeIntE, G4double SCompNuc) {
  G4int i;
...
G4StatMFChannel* G4StatMFMicroManager::ChooseChannel(G4int A0, G4int Z0, G4double MeanT) {
  ...
  for (std::vector<G4StatMFMicroPartition*>::iterator i = _Partition.begin(); i != _Partition.end(); ++i) {
    AccumWeight += (*i)->GetProbability();
    if (RandNumber < AccumWeight) return (*i)->ChooseZ(A0, Z0, MeanT);
  }

  throw G4HadronicException(__FILE__, __LINE__, "G4StatMFMicroCanonical::ChooseChannel: Couldn't find a channel.");
  return 0;
}
