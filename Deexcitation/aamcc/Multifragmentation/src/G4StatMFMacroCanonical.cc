...
void G4StatMFMacroCanonical::Initialize(const G4Fragment& theFragment) {
  ...
  __FreeInternalE0 = A * (-G4StatMFParameters::GetE0() +                  // Volume term (for T = 0)
                     ...
                     0.6 * elm_coupling * Z * Z /
                         (G4StatMFParameters::Getr0() *  // Coulomb term
                          g4calc->Z13(A));
...
void G4StatMFMacroCanonical::CalculateTemperature(const G4Fragment& theFragment) {
  ...
  // Parameter Kappa
  G4Pow* g4calc = G4Pow::GetInstance();
  _Kappa = (1.0 + elm_coupling * (g4calc->A13(FragMult) - 1) / (G4StatMFParameters::Getr0() * g4calc->Z13(A)));
  _Kappa = _Kappa * _Kappa * _Kappa - 1.0;

  G4StatMFMacroTemperature* theTemp = new G4StatMFMacroTemperature(A, Z, U, __FreeInternalE0, _Kappa, &_theClusters);
...
// Determines fragments multiplicities and compute total fragment multiplicity
{
  ...
  for (std::vector<G4VStatMFMacroCluster*>::iterator it = _theClusters.begin() + 1; it != _theClusters.end(); ++it) {
    ...
    for (G4int i = 0; i < multiplicity; i++) {
      ...
      if (A <= 1) {
        ...
        if (RandNumber < (*_theClusters.begin())->GetZARatio()) {
          ...
          FragmentsZ.push_back(0);
      } else {
        G4double RandZ;
        G4double CC = 8.0 * G4StatMFParameters::GetGamma0() + 2 * CP * g4calc->Z23(FragmentsA[i]);
        G4double ZMean;
        if (FragmentsA[i] > 1 && FragmentsA[i] < 5) {
          ZMean = 0.5 * FragmentsA[i];
        } else {
          ZMean = FragmentsA[i] * (4.0 * G4StatMFParameters::GetGamma0() + _ChemPotentialNu) / CC;
        }
