...
G4StatMFMicroCanonical::~G4StatMFMicroCanonical() {
  ...
  if (!_ThePartitionManagerVector.empty()) {
    std::for_each(_ThePartitionManagerVector.begin(), _ThePartitionManagerVector.end(), DeleteFragment());
  }
}
...
void G4StatMFMicroCanonical::Initialize(const G4Fragment& theFragment) {
  ...
  for (G4int im = 2; im <= MaxMult; im++) {
    G4StatMFMicroManager* aMicroManager = new G4StatMFMicroManager(theFragment, im, __FreeInternalE0, SCompoundNucleus);
    _ThePartitionManagerVector.push_back(aMicroManager);
  }

  // W is the total probability
  W = std::accumulate(_ThePartitionManagerVector.begin(), _ThePartitionManagerVector.end(), W,
...
G4double G4StatMFMicroCanonical::CalcFreeInternalEnergy(const G4Fragment& theFragment, G4double T) {
  G4int A = theFragment.GetA_asInt();
  G4int Z = theFragment.GetZ_asInt();
...
// Calculates Temperature and Entropy of compound nucleus
{
  ...
  G4cout << "G4StatMFMicrocanoncal::CalcEntropyOfCompoundNucleus: I can't calculate the temperature" << G4endl;
