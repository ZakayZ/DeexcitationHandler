...
G4double G4StatMFMacroBiNucleon::CalcMeanMultiplicity(const G4double FreeVol, const G4double mu, const G4double nu,
                                                      const G4double T) {
...
G4double G4StatMFMacroBiNucleon::CalcEntropy(const G4double T, const G4double FreeVol) {
  ...
  if (_MeanMultiplicity > 0.0) {
    ...
    Entropy = _MeanMultiplicity *
              (2.5 + G4Log(3.0 * theA * std::sqrt((G4double)theA) * FreeVol / (lambda3 * _MeanMultiplicity)));
  }
  return Entropy;
}
