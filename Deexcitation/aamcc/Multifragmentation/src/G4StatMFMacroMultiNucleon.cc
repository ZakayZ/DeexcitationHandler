...
// Copy constructor
G4StatMFMacroMultiNucleon::G4StatMFMacroMultiNucleon(const G4StatMFMacroMultiNucleon&)
    : G4VStatMFMacroCluster(0)  // Beacuse the def. constr. of base class is private
{
  throw G4HadronicException(__FILE__, __LINE__,
...
G4StatMFMacroMultiNucleon& G4StatMFMacroMultiNucleon::operator=(const G4StatMFMacroMultiNucleon&) {
...
G4double G4StatMFMacroMultiNucleon::CalcMeanMultiplicity(const G4double FreeVol, const G4double mu, const G4double nu,
                                                         const G4double T) {
  ...
  G4double exponent = (mu + nu * theZARatio + G4StatMFParameters::GetE0() + T * T / _InvLevelDensity -
                       G4StatMFParameters::GetGamma0() * (1.0 - 2.0 * theZARatio) * (1.0 - 2.0 * theZARatio)) *
                          theA -
  ...
  _MeanMultiplicity = std::max((FreeVol * theA * std::sqrt((G4double)theA) / lambda3) * G4Exp(exponent), 1.0e-30);
  return _MeanMultiplicity;
}

G4double G4StatMFMacroMultiNucleon::CalcZARatio(const G4double nu) {
  G4double den =
      8 * G4StatMFParameters::GetGamma0() + 2 * G4StatMFParameters::GetCoulomb() * G4Pow::GetInstance()->Z23(theA);
...
G4double G4StatMFMacroMultiNucleon::CalcEnergy(const G4double T) {
  ...
  // Symmetry term
  G4double ESym = theA * G4StatMFParameters::GetGamma0() * (1. - 2. * theZARatio) * (1. - 2. * theZARatio);
...
G4double G4StatMFMacroMultiNucleon::CalcEntropy(const G4double T, const G4double FreeVol) {
  ...
  if (_MeanMultiplicity > 0.0) {
    ...
    // Translational term
    G4double ST = 2.5 + G4Log(FreeVol * std::sqrt((G4double)theA) * theA / (lambda3 * _MeanMultiplicity));
