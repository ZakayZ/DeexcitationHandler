...
G4FermiConfigurationList::G4FermiConfigurationList(const G4FermiConfigurationList&) {
  throw G4HadronicException(__FILE__, __LINE__,
                            "G4FermiConfigurationList::copy_constructor meant to not be accessable");
}
...
G4bool G4FermiConfigurationList::operator==(const G4FermiConfigurationList&) const { return false; }

G4bool G4FermiConfigurationList::operator!=(const G4FermiConfigurationList&) const { return true; }
...
G4bool G4FermiConfigurationList::Initialize(const G4int A, const G4int Z, const G4double TotalEnergyRF) {
  ...
  if (NormStatWeight > 0.0) {
    ...
    std::transform(NormalizedWeights.begin(), NormalizedWeights.end(), NormalizedWeights.begin(),
                   [norm = NormStatWeight](G4double value) { return value / norm; });
    return true;
  }
...
G4FermiConfiguration G4FermiConfigurationList::ChooseConfiguration(void) {
  ...
  if (thisConfig == NormalizedWeights.end()) {
    throw G4HadronicException(__FILE__, __LINE__,
                              "G4FermiConfigurationList::ChooseConfigration: Cannot choose a configuration");
