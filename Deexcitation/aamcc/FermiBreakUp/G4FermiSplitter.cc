...
G4int G4FermiSplitter::Initialize(const G4int a, const G4int z, const G4int n) {
  ...
  if (a < 0 || z < 0 || n < 0) {
    ...
    errOs << "G4FermiSplitter::Initialize() Error: Non valid arguments A = " << a << " Z = " << z
          << " #fragments = " << n;
    throw G4HadronicException(__FILE__, __LINE__, errOs.str());
  }
  if (z > a || n > a) {
    std::ostringstream errOs;
    errOs << "G4FermiSplitter::Initialize() Error: Non physical arguments = " << a << " Z = " << z
          << " #fragments = " << n;
    throw G4HadronicException(__FILE__, __LINE__, errOs.str());
  }
  A = a;
  ...
  do  // for each partition of A
  {
    ...
    do  // for each partition of Z
    {
      ...
      if (static_cast<G4int>(multiplicities.size()) == K) {
        ...
        // Remove wrong splits
        for (std::vector<std::vector<const G4VFermiFragment*> >::iterator itsplits1 = tsplits.begin();
             itsplits1 != tsplits.end(); itsplits1++) {
          std::sort((itsplits1)->begin(), (itsplits1)->end(), std::greater<const G4VFermiFragment*>());
        }
        // add splits  (eliminating a few of them that are repeated)
        std::vector<std::vector<const G4VFermiFragment*> >::iterator itlastsplit = tsplits.begin();
        splits.push_back((*itlastsplit));
        for (std::vector<std::vector<const G4VFermiFragment*> >::iterator itsplits2 = itlastsplit + 1;
             itsplits2 != tsplits.end(); itsplits2++) {
