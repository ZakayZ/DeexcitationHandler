...
G4FragmentVector* G4FermiConfiguration::GetFragments(const G4Fragment& theNucleus) {
  ...
  for (i = Configuration.begin(); i != Configuration.end(); ++i) {
...
#else
    G4LorentzVector FourMomentum(*(MomentumComponents->operator[](std::distance(Configuration.begin(), i))));
#endif
    ...
    for (G4FragmentVector::reverse_iterator ri = fragment->rbegin(); ri != fragment->rend(); ++ri) {
  ...
  if (!MomentumComponents->empty()) {
    std::for_each(MomentumComponents->begin(), MomentumComponents->end(), DeleteFragment());
  }
