...
class G4VFermiFragment {
 public:
  G4VFermiFragment(const G4int anA, const G4int aZ, const G4int Pol, const G4double ExE)
      : A(anA), Z(aZ), Polarization(Pol), ExcitEnergy(ExE) {}
 ...
 public:
  ...
  G4int GetA(void) const { return A; }

  G4int GetZ(void) const { return Z; }

  G4int GetPolarization(void) const { return Polarization; }

  G4double GetExcitationEnergy(void) const { return ExcitEnergy; }

  G4double GetFragmentMass(void) const { return G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(Z, A); }

  G4double GetTotalEnergy(void) const { return this->GetFragmentMass() + this->GetExcitationEnergy(); }
