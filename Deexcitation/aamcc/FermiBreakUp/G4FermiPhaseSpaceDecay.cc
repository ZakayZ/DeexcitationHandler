...
std::vector<G4LorentzVector*>* G4FermiPhaseSpaceDecay::KopylovNBodyDecay(const G4double M,
                                                                         const std::vector<G4double>& m) const
...
std::vector<G4LorentzVector*>* G4FermiPhaseSpaceDecay::NBodyDecay(const G4double M,
                                                                  const std::vector<G4double>& m) const {
  // Number of fragments
  G4int N = m.size();
  G4int i, j;
  ...
  do {
    ...
    std::transform(r.begin(), r.end(), r.begin(), [scale = M - mtot](G4double value) { return value * scale; });
...
std::vector<G4LorentzVector*>* G4FermiPhaseSpaceDecay::TwoBodyDecay(const G4double M,
                                                                    const std::vector<G4double>& m) const {
  G4double m0 = m.front();
  G4double m1 = m.back();
...
// By default Magnitude = 1.0
{
  ...
  G4ParticleMomentum Vector(Magnitude * std::cos(Phi) * SinTheta, Magnitude * std::sin(Phi) * SinTheta,
                            Magnitude * CosTheta);
  return Vector;
}
