...
G4FermiBreakUp::G4FermiBreakUp(const G4FermiBreakUp&) : VFermiBreakUp() {
...
G4bool G4FermiBreakUp::operator==(const G4FermiBreakUp&) const { return false; }

G4bool G4FermiBreakUp::operator!=(const G4FermiBreakUp&) const { return true; }
...
G4FragmentVector* G4FermiBreakUp::BreakItUp(const G4Fragment& theNucleus) {
  ...
  G4bool Split = theConfigurationList.Initialize(static_cast<G4int>(theNucleus.GetA()),
                                                 static_cast<G4int>(theNucleus.GetZ()), TotalEnergyRF);
  if (!Split) {
    G4FragmentVector* theResult = new G4FragmentVector;
    theResult->push_back(new G4Fragment(theNucleus));
